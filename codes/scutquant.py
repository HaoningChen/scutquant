...
# 特征工程
####################################################
def price2ret(price, shift1=-1, shift2=-21, fillna=True):
...
# 自动建模（线性回归模型）
####################################################
def AutoLrg(x, y, method='ols', fit_params=False, alphas=None, logspace_params=None, cv=10, max_iter=1000):
    if alphas is None:
...
def ic_ana(pred, y, groupby=None, freq=1):
